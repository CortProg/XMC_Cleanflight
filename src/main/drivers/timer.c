...
#else
#define _CASE_SHF 12           // amount we can safely shift timer address to the right. gcc will throw error if some timers overlap
#define _CASE_(tim, index) case ((unsigned)tim >> _CASE_SHF): return index; break
...
void timerForceOverflow(TIM_TypeDef *tim)
{
    ...
    ATOMIC_BLOCK(NVIC_PRIO_TIMER) {
        ...
        tim->EGR |= TIM_EGR_UG;
#else
        timerConfig[timerIndex].forcedOverflowTimerValue = ((XMC_CCU4_SLICE_t*)tim)->TIMER;
        XMC_CCU4_SLICE_ClearTimer((XMC_CCU4_SLICE_t*)tim);
#endif
    }
}
